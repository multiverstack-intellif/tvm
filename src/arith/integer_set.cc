...
PrimExpr IntegerSetNode::GenerateConstraint() {
...
TVM_STATIC_IR_FUNCTOR(ReprPrinter, vtable)
    .set_dispatch<IntegerSetNode>([](const ObjectRef& node, ReprPrinter* p) {
      auto* op = static_cast<const IntegerSetNode*>(node.get());
      for (const IntegerRelation &disjunct : op->set.getAllDisjuncts()) {
        assert(disjunct.hasConsistentState());
        p->stream << "Domain: " << op->set.getSpace().getNumDomainVars() << ", "
                  << "Range: " << op->set.getSpace().getNumRangeVars() << ", "
                  << "Symbols: " << op->set.getSpace().getNumSymbolVars() << ", "
                  << "Locals: " << op->set.getSpace().getNumLocalVars() << "\n";
        p->stream << disjunct.getNumConstraints() << " constraints\n";
        for (unsigned i = 0, e = disjunct.getNumEqualities(); i < e; ++i) {
          for (unsigned j = 0, f = disjunct.getNumCols(); j < f; ++j) {
            p->stream << disjunct.atEq(i, j) << " ";
          }
          p->stream << "= 0\n";
        }
        for (unsigned i = 0, e = disjunct.getNumInequalities(); i < e; ++i) {
          for (unsigned j = 0, f = disjunct.getNumCols(); j < f; ++j) {
            p->stream << disjunct.atIneq(i, j) << " ";
          }
          p->stream << ">= 0\n";
        }
        p->stream << '\n';
